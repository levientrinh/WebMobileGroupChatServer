...
function join() {
	...
	if ($('#input_name').val().trim().length <= 0) {
		...
		var messageContainerElement = document.querySelector("#message_container");
...
function send() {
	...
	if (message.trim().length > 0) {
		...
		alert('Vui lòng gõ tin nhắn cần gửi!');
	}
...
function parseMessage(message) {
	...
	if (jObj.flag == 'self') {
		...
		$('p.online_count').html(
				'Chào, <span class="green">' + name + '</span>. <b>'
						+ online_count + '</b> người đang trực tuyến')
				.fadeIn();
